...
class MCP3x0x
{
	...
	static constexpr uint16_t resp_mask = (1u << B) - 1;
	static constexpr uint16_t sign_mask = S ? (1u << (B - 1)) : 0; // only used for signed anyway, but w/e

public:
	static constexpr uint16_t raw_max = S ? resp_mask / 2 : resp_mask;
	static constexpr uint16_t raw_min = S ? resp_mask : 0;
...
private:
	int16_t get_response(spi_transaction_t *trx)
	{
		esp_err_t ret = ESP_OK;
		...
		ESP_GOTO_ON_ERROR(
			spi_device_transmit(spi_hdl, trx),
			errgr, TAG, "Error in get_response()");

		if constexpr (B <= 8)
		{
			bytes[0] = trx->rx_data[0];
			recv >>= 8 - B;
		}
		else
		{
			bytes[1] = trx->rx_data[0];
			bytes[0] = trx->rx_data[1];
			recv >>= 16 - B;
		}
		...
		if constexpr (S == MCP_DATA_SIGNED) // if signed
			if (recv & sign_mask)			// if negative
				recv |= ~resp_mask;			// flip high sign bits
